...
- Current state: initial delivery is ready; post-window recheck result and final receipt ref are intentionally not invented.
...
After delivery, the expected sequence is:

- Frantic runs the inline checks for `evidence_json_valid`, `evidence_items`, `artifact_summary`, `public_url_admitted`, and `report_depth`.
...
- Immediate-pass record: to be copied from Frantic's verifier run after this delivery starts machine verification.
